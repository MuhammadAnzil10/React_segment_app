import SegmentHeader from "./SegementHeaser.jsx";
...
const SegmentForm = () => {
  ...
  const [otherSchemas, setOtherSchemas] = useState();

  const handlChange = async (e) => {
    const selectedLabel = e.target.options[e.target.selectedIndex].id;
    setSelectedSchema({ label: selectedLabel, value: e.target.value });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setSegmentOptions(
      segmentOptions.filter((schema) => selectedSchema.label !== schema.label)
    );
    setSelectedSchema("");
  };
